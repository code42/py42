...
def get_normalized_security_event_plan_info(security_client, user_uid, catch=None):
    ...
    if response is not None:
        ...
        if destination_list:
            ...
            archive_locations_by_plan = {
                plan_uid: [{"destinationGuid": destination}] for destination in destination_list
            }
        else:
            destinations = util.get_obj_from_response(response, "securityPlanLocationsByDestination")
                ...
                for node in destination["securityPlanLocationsByNode"]:
                    for plan_uid in node["securityPlanUids"]:
                        ...
                        if plan_locations is None:
                            ...
                            plan_locations = archive_locations_by_plan.get(plan_uid)
                        plan_locations.append({"destinationGuid": destination["destinationGuid"],
