...
class SessionFactory(BaseSessionFactory):
    ...
    def __init__(self, session_impl, session_modifier_factory, auth_handler_factory, is_async=False):
    ...
    def create_basic_auth_session(self, host_address, username, password):
        ...
        header_modifier = self._session_modifier_factory.create_header_modifier("Basic {0}")
        return self._create_session(self._session_impl, provider, header_modifier, is_async=self._is_async)

    def create_v1_session(self, parent_session, *args, **kwargs):
        provider = C42ApiV1TokenProvider(parent_session)
        header_modifier = self._session_modifier_factory.create_header_modifier("token {0}")
        return self._create_session(self._session_impl, provider, header_modifier, is_async=self._is_async)
    ...
    def create_storage_session(self, c42_api_login_provider, *args, **kwargs):
        header_modifier = self._session_modifier_factory.create_header_modifier("login_token {0}")
        tmp = self._create_session(self._session_impl, c42_api_login_provider, header_modifier, is_async=self._is_async)
...
class SessionModifierFactory(object):
    ...
    def create_v3_session_modifier(self):
        header_modifier = self.create_header_modifier("v3_user_token {0}")
