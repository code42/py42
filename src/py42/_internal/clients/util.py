...
def for_each_api_item(
    ...
    obj_retriever,
    foreach_page_size,
    foreach_user_callback=None,
    foreach_datakey=None,
    foreach_return_each_page=False,
    **kwargs
):
    total_count = util.get_obj_from_response(init_response, u"totalCount")
    _foreach_response(
        init_response, foreach_user_callback, foreach_datakey, foreach_return_each_page, **kwargs
    )
    ...
    if pages > 1:
        for i in range(1, pages):  # skip the first page since we already handled that above
            ...
            def get_page(response):
                _foreach_response(
                    response,
                    foreach_user_callback,
                    foreach_datakey,
                    foreach_return_each_page,
                    **kwargs
                )
...
def _foreach_response(
    response, foreach_user_callback, foreach_datakey, foreach_return_each_page, **kwargs
):
