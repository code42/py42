            ...
            if destination_guid is None:
                ...
                if destination_guid is None:
                    ...
                    if not destination_list:
                        raise Exception(u"No destinations found for device guid: {0}".format(device_guid))
                    destination_guid = destination_list[0][u"targetComputerGuid"]
        except Exception as ex:
            message = u"An error occurred while trying to determine a destination for device guid: {0}," \
