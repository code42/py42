...
def create_eq_filter_group(term, value):
    ...
    equals the given value. Useful for creating ``IS`` filters that are not yet supported in py42
...
def create_is_in_filter_group(term, value_list):
    """"Creates a :class:`FilterGroup` for filtering results where the value with key ``term``
    is one of several values. Useful for creating ``IS_IN`` filters that are not yet supported in
