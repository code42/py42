...
class TestStorageArchiveClient(object):
    def test_search_paths_calls_get_with_expected_params(self, cnxn):
        storage_archive_client = StorageArchiveClient(cnxn)
    ...
    def test_get_file_size_calls_get_with_expected_params(self, cnxn):
        storage_archive_client = StorageArchiveClient(cnxn)
    ...
    def test_get_file_path_metadata_calls_get_with_expected_params(self, cnxn):
        storage_archive_client = StorageArchiveClient(cnxn)
    ...
    def test_create_restore_session_calls_post_with_correct_url(
        ...
        cnxn.post.assert_called_once_with(
    ...
    def test_create_restore_session_with_device_guid_calls_post_with_device_guid_in_json(
        ...
        storage_archive_client.create_restore_session(DEVICE_GUID)
        json_arg = cnxn.post.call_args[KWARGS_INDEX][JSON_KEYWORD]
    ...
    def test_start_restore_calls_post_with_correct_url(self, cnxn):
        storage_archive_client = StorageArchiveClient(cnxn)

        storage_archive_client.start_restore(
            DEVICE_GUID, WEB_RESTORE_SESSION_ID, PATH_SET, NUM_FILES, NUM_DIRS, SIZE
        )
        assert cnxn.post.call_args[ARGS_INDEX][ARGS_INDEX] == WEB_RESTORE_JOB_URL
    ...
    def test_get_restore_status_calls_get_with_correct_url(self, mocker, cnxn):
        storage_archive_client = StorageArchiveClient(cnxn)
        api_response = mocker.MagicMock(spec=Py42Response)
        cnxn.get.return_value = api_response
        storage_archive_client.get_restore_status(WEB_RESTORE_JOB_ID)
        expected_url = WEB_RESTORE_JOB_URL + "/" + WEB_RESTORE_JOB_ID
        cnxn.get.assert_called_once_with(expected_url)

    def test_cancel_restore_calls_delete_with_correct_url(self, mocker, cnxn):
        storage_archive_client = StorageArchiveClient(cnxn)
        api_response = mocker.MagicMock(spec=Py42Response)
        cnxn.delete.return_value = api_response
        storage_archive_client.cancel_restore(WEB_RESTORE_JOB_ID)
        cnxn.delete.assert_called_once_with(WEB_RESTORE_JOB_URL, json=mocker.ANY)

    def test_cancel_restore_calls_delete_with_job_id_in_data(self, mocker, cnxn):
        storage_archive_client = StorageArchiveClient(cnxn)
        api_response = mocker.MagicMock(spec=Py42Response)
        cnxn.delete.return_value = api_response
        storage_archive_client.cancel_restore(WEB_RESTORE_JOB_ID)
        json_arg = cnxn.delete.call_args[KWARGS_INDEX][JSON_KEYWORD]
    ...
    def test_stream_restore_result_status_calls_get_with_correct_url(
        ...
        cnxn.get.assert_called_once_with(expected_url, stream=True)
