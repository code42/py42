...
EXCEPTIONS = [{"path": "/dir/file.ext", TIMESTAMP_KEY: TIMESTAMP},
...
class TestStorageArchiveClient(object):
    ...
    def test_create_web_restore_session_with_device_guid_calls_post_with_device_guid_in_json(self, session):
        storage_archive_client = StorageArchiveClient(session)
        storage_archive_client.create_web_restore_session(DEVICE_GUID)
        json_arg = session.post.call_args[KWARGS_INDEX][JSON_KEYWORD]
        assert json_arg.get(COMPUTER_GUID_KEY) == DEVICE_GUID

    def test_create_web_restore_session_with_data_key_token_calls_post_with_data_key_token_in_json(self, session):
        storage_archive_client = StorageArchiveClient(session)
        storage_archive_client.create_web_restore_session(DEVICE_GUID, data_key_token=DATA_KEY_TOKEN)
        json_arg = session.post.call_args[KWARGS_INDEX][JSON_KEYWORD]
        assert json_arg.get(DATA_KEY_TOKEN_KEY) == DATA_KEY_TOKEN

    def test_create_web_restore_session_with_private_password_calls_post_with_private_password_in_json(self, session):
        storage_archive_client = StorageArchiveClient(session)
        storage_archive_client.create_web_restore_session(DEVICE_GUID, private_password=PRIVATE_PASSWORD)
        json_arg = session.post.call_args[KWARGS_INDEX][JSON_KEYWORD]
        assert json_arg.get(PRIVATE_PASSWORD_KEY) == PRIVATE_PASSWORD

    def test_create_web_restore_session_with_encryption_key_calls_post_with_encryption_key_in_json(self, session):
        storage_archive_client = StorageArchiveClient(session)
        storage_archive_client.create_web_restore_session(DEVICE_GUID, encryption_key=ENCRYPTION_KEY)
        json_arg = session.post.call_args[KWARGS_INDEX][JSON_KEYWORD]
    ...
    def test_submit_web_restore_job_with_required_args_calls_post_with_all_args_in_json(self, session):
        storage_archive_client = StorageArchiveClient(session)
        storage_archive_client.submit_web_restore_job(DEVICE_GUID, WEB_RESTORE_SESSION_ID, PATH_SET, NUM_FILES,
                                                      NUM_DIRS, SIZE)
        json_arg = session.post.call_args[KWARGS_INDEX][JSON_KEYWORD]
    ...
    def test_submit_web_restore_job_with_opt_zip_result_as_false_calls_post_with_zip_result_in_data(self, session):
        storage_archive_client = StorageArchiveClient(session)
        storage_archive_client.submit_web_restore_job(DEVICE_GUID, WEB_RESTORE_SESSION_ID, PATH_SET, NUM_FILES,
                                                      NUM_DIRS, SIZE, zip_result=False)
        json_arg = session.post.call_args[KWARGS_INDEX][JSON_KEYWORD]
        assert json_arg.get(ZIP_RESULT_KEY) is False

    def test_submit_web_restore_job_with_opt_zip_result_as_true_calls_post_with_zip_result_in_data(self, session):
        storage_archive_client = StorageArchiveClient(session)
        storage_archive_client.submit_web_restore_job(DEVICE_GUID, WEB_RESTORE_SESSION_ID, PATH_SET, NUM_FILES,
                                                      NUM_DIRS, SIZE, zip_result=True)
        json_arg = session.post.call_args[KWARGS_INDEX][JSON_KEYWORD]
        assert json_arg.get(ZIP_RESULT_KEY) is True

    def test_submit_web_restore_job_with_expire_job_as_true_calls_post_with_expire_job_in_data(self, session):
        storage_archive_client = StorageArchiveClient(session)
        storage_archive_client.submit_web_restore_job(DEVICE_GUID, WEB_RESTORE_SESSION_ID, PATH_SET, NUM_FILES,
                                                      NUM_DIRS, SIZE, expire_job=True)
        json_arg = session.post.call_args[KWARGS_INDEX][JSON_KEYWORD]
        assert json_arg.get(EXPIRE_JOB_KEY) is True

    def test_submit_web_restore_job_with_expire_job_as_false_calls_post_with_expire_job_in_data(self, session):
        storage_archive_client = StorageArchiveClient(session)
        storage_archive_client.submit_web_restore_job(DEVICE_GUID, WEB_RESTORE_SESSION_ID, PATH_SET, NUM_FILES,
                                                      NUM_DIRS, SIZE, expire_job=False)
        json_arg = session.post.call_args[KWARGS_INDEX][JSON_KEYWORD]
        assert json_arg.get(EXPIRE_JOB_KEY) is False

    def test_submit_web_restore_job_with_show_deleted_true_calls_post_with_show_deleted_in_data(self, session):
        storage_archive_client = StorageArchiveClient(session)
        storage_archive_client.submit_web_restore_job(DEVICE_GUID, WEB_RESTORE_SESSION_ID, PATH_SET, NUM_FILES,
                                                      NUM_DIRS, SIZE, show_deleted=True)
        json_arg = session.post.call_args[KWARGS_INDEX][JSON_KEYWORD]
        assert json_arg.get(SHOW_DELETED_KEY) is True

    def test_submit_web_restore_job_with_show_deleted_false_calls_post_with_show_deleted_false_in_data(self, session):
        storage_archive_client = StorageArchiveClient(session)
        storage_archive_client.submit_web_restore_job(DEVICE_GUID, WEB_RESTORE_SESSION_ID, PATH_SET, NUM_FILES,
                                                      NUM_DIRS, SIZE, show_deleted=False)
        json_arg = session.post.call_args[KWARGS_INDEX][JSON_KEYWORD]
        assert json_arg.get(SHOW_DELETED_KEY) is False

    def test_submit_web_restore_job_with_restore_full_path_true_calls_post_with_restore_full_path_true_in_data(self,
                                                                                                               session):
        storage_archive_client = StorageArchiveClient(session)
        storage_archive_client.submit_web_restore_job(DEVICE_GUID, WEB_RESTORE_SESSION_ID, PATH_SET, NUM_FILES,
    ...
    def test_submit_web_restore_job_with_restore_full_path_false_calls_post_with_restore_full_path_true_in_data(self,
        ...
        storage_archive_client = StorageArchiveClient(session)
        storage_archive_client.submit_web_restore_job(DEVICE_GUID, WEB_RESTORE_SESSION_ID, PATH_SET, NUM_FILES,
                                                      NUM_DIRS, SIZE, restore_full_path=False)
        json_arg = session.post.call_args[KWARGS_INDEX][JSON_KEYWORD]
    ...
    def test_submit_web_restore_job_with_timestamp_calls_post_with_timestamp_in_data(self, session):
        storage_archive_client = StorageArchiveClient(session)
        storage_archive_client.submit_web_restore_job(DEVICE_GUID, WEB_RESTORE_SESSION_ID, PATH_SET, NUM_FILES,
                                                      NUM_DIRS, SIZE, timestamp=TIMESTAMP)
        json_arg = session.post.call_args[KWARGS_INDEX][JSON_KEYWORD]
        assert json_arg.get(TIMESTAMP_KEY) == TIMESTAMP

    def test_submit_web_restore_job_with_exceptions_calls_post_with_exceptions_in_data(self, session):
        storage_archive_client = StorageArchiveClient(session)
        storage_archive_client.submit_web_restore_job(DEVICE_GUID, WEB_RESTORE_SESSION_ID, PATH_SET, NUM_FILES,
                                                      NUM_DIRS, SIZE, exceptions=EXCEPTIONS)
        json_arg = session.post.call_args[KWARGS_INDEX][JSON_KEYWORD]
        assert json_arg.get(EXCEPTIONS_KEY) == EXCEPTIONS

    def test_submit_web_restore_job_with_backup_set_id_calls_post_with_backup_set_id_in_data(self, session):
        storage_archive_client = StorageArchiveClient(session)
        storage_archive_client.submit_web_restore_job(DEVICE_GUID, WEB_RESTORE_SESSION_ID, PATH_SET, NUM_FILES,
                                                      NUM_DIRS, SIZE, backup_set_id=BACKUP_SET_ID)
        json_arg = session.post.call_args[KWARGS_INDEX][JSON_KEYWORD]
    ...
    def test_submit_web_restore_job_with_all_args_calls_post_with_all_args_in_data(self, session):
        storage_archive_client = StorageArchiveClient(session)
        storage_archive_client.submit_web_restore_job(DEVICE_GUID, WEB_RESTORE_SESSION_ID, PATH_SET, NUM_FILES,
                                                      NUM_DIRS, SIZE, zip_result=True, expire_job=True,
                                                      show_deleted=True, restore_full_path=True, timestamp=TIMESTAMP,
                                                      exceptions=EXCEPTIONS, backup_set_id=BACKUP_SET_ID)
        json_arg = session.post.call_args[KWARGS_INDEX][JSON_KEYWORD]
        expected_data = {GUID_KEY: DEVICE_GUID,
    ...
    def test_cancel_web_restore_job_with_kwargs_calls_delete_with_kwargs(self, session):
        storage_archive_client = StorageArchiveClient(session)
        storage_archive_client.cancel_web_restore_job(WEB_RESTORE_JOB_ID, kwarg_1=True, kwarg_2="val")
    ...
    def test_get_web_restore_job_result_with_kwargs_calls_get_with_kwargs(self, session):
        storage_archive_client = StorageArchiveClient(session)
        storage_archive_client.get_web_restore_job_result(WEB_RESTORE_JOB_ID, kwarg_1=True, kwarg_2="val")
