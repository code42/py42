...
def test_create_is_in_filter_group_calls_create_file_event_filter_with_correct_values(mocker):
    ...
    op = "IS"
    calls = [mocker.call(term, op, "isinval1"), mocker.call(term, op, "isinval2"), mocker.call(term, op, "isinval3")]

    py42._internal.file_event_filter.create_file_event_filter.assert_has_calls(calls, any_order=True)
...
def test_create_not_in_filter_group_calls_create_file_event_filter_with_correct_values(mocker):
    ...
    op = "IS_NOT"
    calls = [mocker.call(term, op, "notinval1"), mocker.call(term, op, "notinval2"), mocker.call(term, op, "notinval3")]

    py42._internal.file_event_filter.create_file_event_filter.assert_has_calls(calls, any_order=True)


def test_create_on_or_before_filter_group_calls_create_file_event_filter_with_correct_values(mocker):
    mocker.patch("py42._internal.file_event_filter.create_file_event_filter")
...
def test_create_in_range_filter_group_calls_create_file_event_filter_with_correct_values(mocker):
    ...
    calls = [mocker.call(term, "ON_OR_BEFORE", end_time), mocker.call(term, "ON_OR_AFTER", start_time)]
...
def test_create_eq_filter_group_returns_obj_with_correct_json_representation():
    ...
    assert str(filter_group) == '{"filterClause":"AND",' \
...
def test_create_is_in_filter_group_returns_obj_with_correct_json_representation():
    ...
    assert str(filter_group) == '{"filterClause":"OR",' \
...
def test_create_file_event_filter_returns_obj_with_correct_json_representation():
    file_event_filter = create_file_event_filter(EVENT_FILTER_FIELD_NAME, OPERATOR_STRING, VALUE_STRING)
    assert str(file_event_filter) == '{{"operator":"{0}", "term":"{1}", "value":"{2}"}}'.format(OPERATOR_STRING,
                                                                                                EVENT_FILTER_FIELD_NAME,
                                                                                                VALUE_STRING)
