...
class TestPy42Session(object):
    ...
    def test_session_post_with_json_calls_filter_out_none_util(self, mocker, success_requests_session):
    ...
    def test_session_post_with_data_and_json_params_overwrites_data_with_json(self, success_requests_session):
        session = Py42Session(success_requests_session, HOST_ADDRESS)
        session.post(URL, data=DATA_VALUE, json=JSON_VALUE)
        assert success_requests_session.request.call_args[KWARGS_INDEX][DATA_KEY] == dumps(JSON_VALUE)

    def test_session_post_with_data_and_json_params_does_not_pass_json_param_to_request(self, success_requests_session):
        session = Py42Session(success_requests_session, HOST_ADDRESS)
        session.post(URL, data=DATA_VALUE, json=JSON_VALUE)
    ...
    def test_request_upon_exception_url_included_in_message(
        ...
        with pytest.raises(HTTPError) as e:
            session.get(URL)
        expected = build_expected_exception_message(HOST_ADDRESS, URL, HTTPError, REQUEST_EXCEPTION_MESSAGE)
    ...
    def test_request_with_catch_upon_exception_calls_catch_with_exception_message(
        ...
        assert str(exception) == build_expected_exception_message(HOST_ADDRESS, URL, HTTPError, REQUEST_EXCEPTION_MESSAGE)
    ...
    def test_request_without_catch_upon_exception_calls_global_receiver_with_message_and_trace(
            self, error_requests_session, renewing_auth_handler, http_error, catch, global_exception_message_receiver, traceback):
