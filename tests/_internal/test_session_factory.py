...
class TestSessionFactory(object):
    ...
    def test_create_file_event_session_calls_session_constructor_with_correct_args(self, mocker,
        ...
        mock_call_session_constructor = mocker.patch('py42._internal.session_factory.call_session_constructor')

        factory = SessionFactory(Session, session_modifier_factory, auth_handler_factory)
        factory.create_file_event_session(file_event_login_provider)

        mock_call_session_constructor.assert_called_once_with(Py42Session, Session, TARGET_HOST_ADDRESS, auth_handler)
    ...
    def test_create_file_event_session_uses_auth_handler_with_provider_and_modifier(self, mocker,
        ...
        factory = SessionFactory(Session, session_modifier_factory, auth_handler_factory)
        factory.create_file_event_session(file_event_login_provider)
        auth_handler_factory.create_auth_handler.assert_called_once_with(file_event_login_provider, modifier)

    def test_create_file_event_session_with_is_async_true_creates_async_session(self, mocker,
                                                                                   session_modifier_factory,
                                                                                   auth_handler_factory,
                                                                                   file_event_login_provider):
        ...
        mock_call_session_constructor.assert_called_once_with(Py42AsyncSession, Session, TARGET_HOST_ADDRESS, auth_handler)
