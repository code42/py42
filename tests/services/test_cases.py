...
class TestCasesService():
    ...
    def test_create_called_with_expected_url_and_params(self, mock_connection):
        ...
        cases_service.create(u"name", u"subject", u"user uid", u"description", u"findings")
    ...
    def test_get_all_called_with_expected_url_and_params(self, mock_connection):
        cases_service = CasesService(mock_connection)
        mock_connection.get.side_effect = [DUMMY_GET_ALL_TEST_RESPONSE, DUMMY_GET_ALL_TEST_RESPONSE]
        for _ in cases_service.get_all():
            continue

        mock_connection.get.call_count == 2
        assert mock_connection.get.call_args[0][0] == u"/api/v1/case?pgNum=1&pgSize=500&srtDir=asc&srtKey=number"
    ...
    def test_get_case_by_case_number_called_with_expected_url_and_params(self, mock_connection):
        cases_service = CasesService(mock_connection)
