...
@pytest.mark.parametrize(
    "level",
    [debug_level.INFO, debug_level.DEBUG, debug_level.TRACE],
    ids=["INFO", "DEBUG", "TRACE"],
)
...
@pytest.mark.parametrize(
    "level", [debug_level.NONE, debug_level.INFO, debug_level.DEBUG], ids=["NONE", "INFO", "DEBUG"]
)
...
@pytest.mark.parametrize(
    "level",
    [debug_level.NONE, debug_level.INFO, debug_level.DEBUG, debug_level.TRACE],
...
@pytest.mark.parametrize("level", [-1, 4, 10])
def test_will_print_for_given_debug_level_enabled_with_undefined_level_returns_false(
    debug_enabled, level
):
    assert not debug.will_print_for(level)
